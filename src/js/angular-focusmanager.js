...
(function(exports, global) {
    ...
    module.directive("focusGroup", [ "focusManager", "focusQuery", "focusDispatcher", "focusKeyboard", function(focusManager, focusQuery, focusDispatcher, focusKeyboard) {
        var groupId = 1, elementId = 1, dispatcher = focusDispatcher(), delay = 100, orientation;
        ...
        function linker(scope, element, attr) {
            ...
            var outOfBody = false;
            var orientation = attr.focusGroup === "horizontal" ? consts.ORIENTATION.HORIZONTAL : consts.ORIENTATION.VERTICAL;
            var focusInOff, focusEnabledOff, disabledOff;
            focusManager.orientations = focusManager.orientations || {};
            focusManager.orientations[groupName] = orientation;
            function init() {
    ...
    module.service("focusManager", [ "focusQuery", "focusDispatcher", function(focusQuery, focusDispatcher) {
        ...
        function focus(el) {
            ...
            if (scope.activeElement !== el) {
                ...
                utils.removeClass(scope.activeElement, "focused");
                scope.activeElement = el;
                if (el) {
                    el.focus();
                }
                dispatcher.trigger("focusin", eventObj);
                utils.addClass(el, "focused");
            }
        }
        ...
        function next() {
            var groupId, elementId, group, parentGroupId;
            ...
            if (activeElement) {
                ...
                isVertical = scope.orientations[groupId] === consts.ORIENTATION.VERTICAL;
                if (isVertical) {
        ...
        function up() {
            var groupId, group, elementId, parentGroupId;
            var activeElement = getActiveElement();
            var isVertical;
            if (activeElement) {
                groupId = focusQuery.getParentId(activeElement);
        ...
        function getClosestElement(elements) {
            var curr = getActiveElement(), currOffset, element, elementOffset, minDistance, distance;
            var direction = scope.direction;
            var condition;
            if (curr) {
                currOffset = curr.getBoundingClientRect();
                for (var i=0; i<elements.length; i++) {
                    elementOffset = elements[i].getBoundingClientRect();
                    distance = Math.pow(Math.pow(elementOffset.left - currOffset.left, 2) + Math.pow(elementOffset.top - currOffset.top, 2), 0.5);
                    minDistance = minDistance === undefined ? distance : minDistance;
        ...
        function getClosestGroup(groups) {
            var group, distance;
            for (var i=0; i<groups.length; i++) {
                distance = groups[i].getBoundingClientRect().left;
                if (distance >= 0) {
                    group = groups[i];
                    break;
                }
        ...
        function getPrevElement(elements, elementId) {
            ...
            if (elements && elements.length) {
                if (elementId) {
                    ...
                    return getClosestElement(elements);
                }
            }
        }
                ...
                if (groupId) {
                    ...
                    if (index > 0) {
                        if (scope.direction === "up") {
                            if (groups[index - 1].getBoundingClientRect().bottom < getActiveElement().getBoundingClientRect().top) {
                                return groups[index - 1];
                            }
                        } else {
                            if (groups[index - 1].getBoundingClientRect().right < getActiveElement().getBoundingClientRect().left) {
                                return groups[index - 1];
                            }
                        }
                    }
            ...
            if (groups) {
                ...
                if (index !== -1 && index + 1 < groups.length) {
                    if (scope.direction === "down") {
                        if (groups[index + 1].getBoundingClientRect().top > getActiveElement().getBoundingClientRect().top) {
                            return groups[index + 1];
                        }
                    } else {
                        if (groups[index + 1].getBoundingClientRect().left > getActiveElement().getBoundingClientRect().left) {
                            return groups[index + 1];
                        }
                    }
                }
        ...
        function findNextChildGroup(groupId) {
            var groups, group, nextGroup, nextGroupId, parentGroupId;
            groups = focusQuery.getChildGroups(groupId);
            if (groups.length) {
                nextGroup = getClosestGroup(groups);
                nextGroupId = focusQuery.getGroupId(nextGroup);
        ...
        function findPrevChildGroup(groupId) {
            var groups, childGroup, childGroupId;
            if (groupId) {
                groups = focusQuery.getChildGroups(groupId);
                if (groups.length) {
                    childGroup = scope.direction === "prev" ? groups[groups.length - 1] : groups[0];
                    childGroupId = focusQuery.getGroupId(childGroup);
    ...
    utils.addClass = function (el, klass) {
        el && !el.classList.contains(klass) && el.classList.add(klass);
    };
    utils.removeClass = function (el, klass) {
        el && el.classList.contains(klass) && el.classList.remove(klass);
    };
    (function(J, r, f) {
        function s(a, b, d) {
